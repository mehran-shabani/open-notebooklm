  ...
  return (
    <div className={cn('flex-shrink-0 flex items-center justify-between gap-2 py-2 px-3 border-t bg-muted/30', className)}>
      <div className="flex items-center gap-2">
        <span className="text-xs font-medium text-muted-foreground">{t('common.context.label')}</span>
        ...
        <div className="flex items-center gap-1.5">
          {sourcesInsights > 0 && (
            <Tooltip>
              ...
              <TooltipContent>
                <p>{t('common.context.insightsSources', { count: sourcesInsights })}</p>
              </TooltipContent>
            </Tooltip>
          )}
          ...
          {sourcesFull > 0 && (
            <Tooltip>
              ...
              <TooltipContent>
                <p>{t('common.context.fullSources', { count: sourcesFull })}</p>
              </TooltipContent>
            </Tooltip>
          )}
        </div>
        ...
        {notesCount > 0 && (
          <>
            ...
            <Tooltip>
              ...
              <TooltipContent>
                <p>{t('common.context.fullNotes', { count: notesCount })}</p>
              </TooltipContent>
            </Tooltip>
          </>
        )}
      ...
      {(tokenCount !== undefined || charCount !== undefined) && (
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          {tokenCount !== undefined && tokenCount > 0 && (
            <span>{t('common.context.tokens', { count: formatNumber(tokenCount) })}</span>
          )}
          {tokenCount !== undefined && charCount !== undefined && tokenCount > 0 && charCount > 0 && (
            <span>/</span>
          )}
          {charCount !== undefined && charCount > 0 && (
            <span>{t('common.context.chars', { count: formatNumber(charCount) })}</span>
          )}
        </div>
      )}
    </div>
